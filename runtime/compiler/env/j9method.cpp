...
bool
TR_J9MethodBase::isVarHandleAccessMethod(TR::RecognizedMethod rm)
   {
   switch (rm)
      {
...
bool
TR_J9MethodBase::isSignaturePolymorphicMethod(TR::RecognizedMethod rm)
   {
   if (isVarHandleOperationMethod(rm)) return true;
...
TR_ResolvedJ9Method::forceCompileTimeResolveMethod(I_32 cpIndex)
   {
   // Check if the method is among those that has to be compile-time resolved
...
TR_ResolvedJ9Method::createResolvedMethodFromJ9Method(TR::Compilation *comp, I_32 cpIndex, uint32_t vTableSlot, J9Method *j9Method, bool * unresolvedInCP, TR_AOTInliningStats *aotStats)
   ...
   if (TR_J9MethodBase::isSignaturePolymorphicMethod(m->getRecognizedMethod()))
      {
