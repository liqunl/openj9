...
TR_J9EstimateCodeSize::realEstimateCodeSize(TR_CallTarget *calltarget, TR_CallStack *prevCallStack, bool recurseDown)
   ...
   if (nph.doPeeking() && recurseDown || calltarget->_calleeMethod->convertToMethod()->isArchetypeSpecimen())
      {
            ...
            if (tt->getNode()->getNumChildren()>0 &&
               ...
               if (!callNode->getSymbolReference()->isUnresolved() && !visited.contains(callNode))
                  ...
                  if (rm == TR::java_lang_invoke_DirectHandle_nullCheckIfRequired ||
                      ...
                      rm == TR::java_lang_invoke_MethodHandle_doCustomizationLogic ||
                      rm == TR::java_lang_invoke_MethodHandle_undoCustomizationLogic)
                     {
                     continue;
                     }
         ...
         switch (bc)
            ...
            case J9BCinvokedynamic:
               ...
               if (knot && !owningMethod->isUnresolvedCallSiteTableEntry(cpIndex))
                  {
                  isIndirectCall = true;
                  TR::SymbolReference *mhSymRef = comp()->getSymRefTab()->findOrCreateCallSiteTableEntrySymbol(methodSymbol, cpIndex);
