...
InterpreterEmulator::computePrexInfo(TR_CallSite *callsite)
   ...
   if (_iteratorWithState)
      ...
      if (callNodeTT)
         {
         // Temporaryly set call tree and call node of callsite such that computePrexInfo can use it
         callsite->_callNodeTreeTop = callNodeTT;
         callsite->_callNode = callNodeTT->getNode()->getChild(0);
         auto prexArgInfo = _ecs->getInliner()->getUtil()->computePrexInfo(callsite, _calltarget->_ecsPrexArgInfo);
