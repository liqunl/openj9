...
TR_MethodHandleTransformer::getMethodEntryObjectInfo()
   ...
   ObjectInfo* objectInfo = new (comp()->trMemory()->currentStackRegion()) ObjectInfo(_numLocals, TR::KnownObjectTable::UNKNOWN, comp()->trMemory()->currentStackRegion());
   if (argInfo)
      {
      TR_ResolvedMethod* currentMethod = comp()->getCurrentMethod();
...
TR_MethodHandleTransformer::blockStartObjectInfoFromPredecessors(TR::Block* block)
   ...
   for (TR::CFGEdge *edge = pi.getFirst(); edge != NULL; edge = pi.getNext())
      ...
      if (_blockEndObjectInfos->find(fromBlockNum) == _blockEndObjectInfos->end())
         ...
         return new (comp()->trMemory()->currentStackRegion()) ObjectInfo(_numLocals, TR::KnownObjectTable::UNKNOWN, comp()->trMemory()->currentStackRegion());
         }
      }
   ...
   for (TR::CFGEdge *edge = pi.getFirst(); edge != NULL; edge = pi.getNext())
      ...
      if (!objectInfo)
         objectInfo = new (comp()->trMemory()->currentStackRegion()) ObjectInfo(*fromBlockObjectInfo, comp()->trMemory()->currentStackRegion());
      else
