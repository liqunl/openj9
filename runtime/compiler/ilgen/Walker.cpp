...
TR_J9ByteCodeIlGenerator::genInvokeDynamic(int32_t callSiteIndex)
   ...
   if (!owningMethod->isUnresolvedCallSiteTableEntry(callSiteIndex))
      {
      TR_ResolvedMethod *specimen = fej9()->createMethodHandleArchetypeSpecimen(trMemory(), (uintptrj_t*)owningMethod->callSiteTableEntryAddress(callSiteIndex), owningMethod, true);
      if (specimen)
...
TR_J9ByteCodeIlGenerator::genHandleTypeCheck(TR::Node* handle, TR::Node* expectedType)
   {
   uint32_t typeOffset =  fej9()->getMethodHandleTypeOffset(comp());
   TR::SymbolReference *typeSymRef = comp()->getSymRefTab()->findOrFabricateShadowSymbol(_methodSymbol, TR::Symbol::Java_lang_invoke_MethodHandle_type, TR::Address, typeOffset, false, false, true, "java/lang/invoke/MethodHandle.type Ljava/lang/invoke/MethodType;");
